...
function Verifier() {
  ...
  if (result.status === 'verifying') {
    return (
      <div className="w-full max-w-md mx-4 text-center">
        <div className="bg-card/50 backdrop-blur-md p-8 rounded-2xl shadow-2xl border border-muted">
          <h2 className="text-2xl font-bold text-foreground mb-2">{loadingState.headline}</h2>
          <p className="text-muted mb-6">{loadingState.message}</p>
  ...
  return (
    <main className={`w-full max-w-2xl mx-4 transition-opacity duration-500 ${showResult ? 'opacity-100' : 'opacity-0'}`}>
      <div className="bg-card/60 backdrop-blur-lg rounded-2xl shadow-2xl p-6 md:p-10 border border-muted text-foreground">
        ...
        <header className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <div className="text-sm text-muted">Official Certificate Verification</div>
          </div>
        </header>

        <div className="bg-card/70 rounded-lg p-6 border border-muted">
          <div className={`flex items-center gap-2 font-bold text-sm px-4 py-2 rounded-full mb-6 w-fit ${isSuccess ? 'bg-primary/20 text-primary' : 'bg-destructive/20 text-destructive'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-8 text-left">
            <div>
              <div className="text-xs uppercase font-semibold text-muted tracking-wider">Student</div>
              <div className="text-lg font-medium text-foreground mt-1 break-words">{result.data?.studentName ?? '—'}</div>
            </div>
            <div>
              <div className="text-xs uppercase font-semibold text-muted tracking-wider">Program / Domain</div>
              <div className="text-lg font-medium text-foreground mt-1 break-words">{result.data?.domain ?? '—'}</div>
            </div>
            <div>
              <div className="text-xs uppercase font-semibold text-muted tracking-wider">Date of Issue</div>
              <div className="text-lg font-medium text-foreground mt-1 break-words">{result.data?.issueDate ?? result.message ?? '—'}</div>
            </div>
          </div>
        </div>

        <p className="text-center text-xs text-muted mt-8">Verification data is sourced directly from our secure registry.</p>
      </div>
    </main>
  );
}
...
const LoadingFallback = () => (
    <div className="w-full max-w-md mx-4 text-center">
        <div className=" p-8 rounded-2xl shadow-2xl border border-muted">
            <h2 className="text-2xl font-bold text-foreground">Loading Verifier...</h2>
