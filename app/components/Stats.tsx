...
const StatCard: React.FC<StatCardProps> = ({ number, suffix, label }) => {
  ...
  return (
    <div ref={ref} className="bg-background/50 backdrop-blur-sm p-8 rounded-xl">
      <p className="text-5xl font-bold text-highlight">
        {inView ? <CountUp end={number} duration={2.5} /> : '0'}
        {suffix}
      </p>
      <p className="mt-3 text-lg text-muted-foreground">{label}</p>
    </div>
  );
};
