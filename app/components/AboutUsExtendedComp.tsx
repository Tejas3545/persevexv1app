...
export default function AboutUsExtendedComp({
  ...
  if (isMobile) {
    return (
      <div className="flex flex-col items-center w-full min-h-screen px-4 py-16 pt-32 text-foreground gap-20">
        <motion.div
        ...
        >
          ...
          <p className="text-muted max-w-2xl mx-auto">
            Discover the principles that drive us and the future we're building
            together.
          </p>
        </motion.div>
          ...
          {cardData.map((card, index) => {
            ...
            return (
              <motion.div
                ...
                className="bg-foreground/30 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg overflow-hidden cursor-pointer"
              >
        ...
        <div
          ...
          style={{ height: "1px", width: "1px", pointerEvents: "none" }}
        />
      </div>
    );
  }
  ...
  const expansionProgress = easeOutCubic(
    Math.max(0, Math.min(cascadingProgress / 0.8, 1))
  );
  const cardFadeProgress = easeOutCubic(
    Math.max(0, Math.min((cascadingProgress - 3.0) / 0.5, 1))
  );
  const cardStackOpacity = 1 - cardFadeProgress;

  const statsRiseProgress = easeOutCubic(
    Math.max(0, Math.min((cascadingProgress - 3.2) / 0.5, 1))
  );
    ...
    switch (index) {
      ...
      case 1: {
        if (isCard1Expanded) {
          ...
          const startY = 800,
            endY = -10,
            currentY = startY + (endY - startY) * card1StackingProgress;
          ...
          const fadeProgress = Math.max(
            0,
            Math.min((cascadingProgress - 0.4) / 0.4, 1)
          );
      ...
      case 2: {
        if (isCard2Expanded) {
          ...
          const card2StackingProgress = easeOutCubic(
            Math.max(0, Math.min((cascadingProgress - 2.0) / 0.8, 1))
          );
          width = finalWidth;
          height = finalHeight;
          zIndex = 20;
          ...
          const startY = 800,
            endY = -10,
            currentY = startY + (endY - startY) * card2StackingProgress;
          transform = `translateY(${currentY}px)`;
        } else {
          const translateX = `${110 * unstackedProgress}%`;
          const stackingY = verticalOffset * 2 * easedStackingProgress;
          const stage1Progress = easeOutCubic(Math.min(cascadingProgress, 1)),
            stage1RevealY = stage1Progress * revealDistance;
          const stage2Progress = easeOutCubic(
              Math.max(0, Math.min(cascadingProgress - 1, 1))
            ),
            stage2RevealY = stage2Progress * revealDistance;
          const finalTranslateY =
            stackingY + stage1RevealY * 1.5 + stage2RevealY;
          const fadeProgress = Math.max(
            0,
            Math.min((cascadingProgress - 1.4) / 0.4, 1)
          );
          opacity = 1 - fadeProgress;
  ...
  return (
    <div className="flex flex-col items-center justify-start w-full pt-20 text-foreground">
      <div className="relative w-full max-w-7xl lg:w-full mx-auto px-4 sm:px-6 min-h-[30rem]">
        ...
        >
          {cardData.map((card, index) => {
            ...
            if (index === 0)
              cardExpansionProgress = expansionProgress > 0.99 ? 1 : 0;
            ...
            if (index === 0) {
              ...
              if (cascadingProgress >= 1.0 && !isCard1Expanded) {
                shouldShowContent = false;
              }
            } else if (index === 2) {
              if (cascadingProgress >= 2.0 && !isCard2Expanded) {
                shouldShowContent = false;
              }
            }
            ...
            return (
              <div
                key={index}
                className="bg-foreground/30 backdrop-blur-sm rounded-2xl border border-white/20 shadow-lg p-8 overflow-hidden"
                      ...
                      <AnimatePresence>
                        {cardExpansionProgress === 1 ? (
                          ...
                          <div key={`truncated-${index}`}>
                            ...
                            <p
                              className="text-muted text-sm leading-relaxed flex-1"
                              style={{
                                display: "-webkit-box",
