    ...
    <div className="py-16 sm:py-24">
      ...
      >
        {cardData.map((card, index) => (
          <motion.div
            key={index}
            className="relative bg-card/40 p-8 rounded-3xl overflow-hidden"
            style={{ backgroundImage: dashedBorderUrl }}
            variants={itemVariants}
          >
            <div className="relative z-10">
              <h2 className="text-6xl lg:text-7xl font-extrabold text-foreground">
                {card.stat}
              </h2>
              <p className="mt-2 text-sm font-semibold text-muted tracking-widest uppercase">
                {card.label}
              </p>
            </div>
          </motion.div>
